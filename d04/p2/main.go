...
func processInput(r io.Reader) (int, error) {
	...
	for s.Scan() {
		...
		// Determine if either elves overlap with one another at all
		if e1start >= e2start && e1start <= e2stop {
			count++
			continue
		}
		if e1stop >= e2start && e1stop <= e2stop {
			count++
			continue
		}
		if e2start >= e1start && e2start <= e1stop {
			count++
			continue
		}
		if e2stop >= e1start && e2stop <= e1stop {
			count++
			continue
		}
	}
